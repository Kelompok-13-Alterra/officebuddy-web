...
const Sidebar = ({ clickedLink }) => {
  ...
  return (
    <div className="relative min-h-screen min-w-[240px] max-w-72 pt-8 border-r-2 mx-2 border-gray-300 ">
      ...
      <Link to={"/pendapatan"}>
        <ul
          className={`${
            activeLink === "Pendapatan"
              ? "w-full py-4 bg-slate-100 border-r-4 border-blue-800 text-blue-500"
              : "w-full py-4 text-slate-300 "
          } cursor-pointer`}
          onClick={() => handleLinkClick("Pendapatan")}
        >
          <li className=" flex mx-auto w-full  pl-11 text-start  justify- items-center mr-10  ">
            <svg
              width="28"
            ...
            >
              <path
                ...
                fill={`${activeLink === "Pendapatan" ? "#005DB9" : "#E0E2EC"}`}
              />
            </svg>
            <span className="text-start pl-8 w-full text-sm  font-medium  ">
              Pendapatan
